...
TEXTFIELD_GROUP_VAR = read_all(
    PDF_DATA_DIR + '/minimal-with-textfield-group-var.pdf'
)
MINIMAL_ONE_FIELD_TAGGED = read_all(
    PDF_DATA_DIR + '/minimal-with-field-tagged.pdf'
)
...
MINIMAL_ONE_FIELD_AES256 = read_all(
    PDF_DATA_DIR + '/minimal-with-field-aes256.pdf'
)
MINIMAL_PUBKEY_AES256 = read_all(PDF_DATA_DIR + '/minimal-pubkey-aes256.pdf')
MINIMAL_PUBKEY_ONE_FIELD_AES256 = read_all(
    PDF_DATA_DIR + '/minimal-with-field-pubkey-aes256.pdf'
)
MINIMAL_PUBKEY_RC4 = read_all(PDF_DATA_DIR + '/minimal-pubkey-rc4.pdf')
MINIMAL_PUBKEY_ONE_FIELD_RC4 = read_all(
    PDF_DATA_DIR + '/minimal-with-field-pubkey-rc4.pdf'
)
...
TESTING_CA_ERRORS = CERTOMANCER.get_pki_arch(
    ArchLabel('testing-ca-with-errors')
)
